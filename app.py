...
if uploaded_image is not None:
    ...
    # Jika confidence CNN rendah (misalnya < 0.5), prioritaskan prediksi Random Forest
    if cnn_confidence < 0.5:
        st.write("Confidence CNN rendah, menggunakan prediksi dari Random Forest...")
        rf_features = extract_features_for_rf(image_path)
        rf_pred = rf_model.predict(rf_features)
        rf_class = class_names[rf_pred[0]]
        st.write(f"**Prediksi Akhir dengan Random Forest:** {rf_class}")
    else:
